...
class FaceTrackingSystem:
    ...
    def get_status(self):
        ...
        with self.tracker_lock:
            # The service is considered running if the global flag is set 
            # OR if any camera has been started individually.
            is_running_logically = self.is_running or any(
                status == 'running' for status in self.camera_status.values()
            )
            return {
                "is_service_running": is_running_logically,
    ...
    def stop_camera(self, camera_id: int) -> bool:
        ...
        with self.tracker_lock:
            ...
            if shutdown_event and thread:
                ...
                shutdown_event.set()
                # REMOVED: thread.join(timeout=5)
                # The thread will now shut down gracefully in the background.
    ...
    def _start_all_cameras_worker(self):
        """Worker to start all cameras with a delay in a background thread."""
        print("[SYSTEM] Staggered camera startup initiated in background...")
        for cam_id in self.camera_configs.keys():
            # If a global stop is triggered during startup, cancel the process
            if not self.is_running:
                print("[SYSTEM] Camera startup process cancelled.")
                break
            self.start_camera(cam_id)
            time.sleep(2) # This delay now happens in the background
        print("[SYSTEM] All camera startup signals sent.")

    # REPLACE the old start_tracking function with this one
    def start_tracking(self):
        ...
        with self.tracker_lock:
            ...
            # Set the intended state to running immediately
            self.is_running = True 

            # Create and start a background thread to handle the staggered startup
            startup_thread = threading.Thread(target=self._start_all_cameras_worker, daemon=True)
    ...
    def _frame_processor_thread(self, camera_config):
        ...
        while not self.global_shutdown_flag.is_set():
            try:
                ...
                try:
                    faces = future.result(timeout=2.0)
    ...
    def _process_pending_updates(self, pending_updates):
        with self.embedding_update_lock:
            for identity, embedding, image_bytes, timestamp in pending_updates: # Unpack new image_bytes
                user_record = db_utils.get_user_by_employee_id(identity)
                if user_record:
                    user_id = user_record[0]
                    # Pass the actual image_bytes to the database function
                    new_embedding_id = db_utils.add_face_embedding(user_id, embedding, image_bytes, 'update')
                    if new_embedding_id:
